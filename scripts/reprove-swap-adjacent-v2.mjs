...
const oldRaw = await read('.cache/swap-adjacent-v1-0a68eba/puzzles.json'), old = JSON.parse(oldRaw);
...
await mkdir(new URL('.cache/swap-adjacent-v2-reproof/', root), { recursive: true });
for (let offset = 0; offset < old.length && performance.now() - started < 300000; offset += 100) {
  ...
  await writeFile(new URL(`.cache/swap-adjacent-v2-reproof/batch-${offset}.json`, root), JSON.stringify({ puzzles: puzzles.filter(p => old.slice(offset, offset + 100).some(x => x.id === p.id)), failures, elapsedMs: performance.now() - started }) + '\n');
...
if (puzzles.length !== old.length || failures.length) { console.log(JSON.stringify({ status: 'UNVERIFIED', proved: puzzles.length, failures })); process.exitCode = 1; }
  ...
  let preview = JSON.parse(await read('.cache/swap-adjacent-v1-0a68eba/preview.json'));
